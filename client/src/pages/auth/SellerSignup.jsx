...
export default function SellerSignup() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      await sellerSignup({ ...form, city: form.city.split(",")[0].trim() });
  ...
  return (
    <AuthShell title="Start selling live" subtitle="Set up your store on the growth engine." accent="dark">
      <form onSubmit={handleSubmit} className="space-y-4">
        <FormField label="Your name" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
        <FormField
        ...
        <label className="block">
          ...
          <span className="mt-1 block text-[11px] text-gray-400">Used for your Regional Demand board and nearby-buyer signals.</span>
        </label>
        ...
        <label className="block">
          ...
          <span className="mt-1 block text-[11px] text-gray-400">Your AI Opportunity Feed will be translated into this language.</span>
        </label>
        ...
        <p className="text-center text-sm text-gray-500">
          Already selling?{" "}
          <Link to="/seller/login" className="font-semibold text-brand-600">
            Log in
          </Link>
        </p>
      </form>
